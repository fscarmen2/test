...
type -p $c >/dev/null 2>&1 || (yellow " $(eval echo "安装 $c 中…… " && ${PACKAGE_INSTALL[b]} "$c") || (yellow " $(eval echo "${T[${L}8]}") " && ${PACKAGE_UPDATE[b]} && ${PACKAGE_INSTALL[b]} "$c")
...
if [[ $ARCHITECTURE = i386 ]]; then
  b=0;PACKAGE_INSTALL=("brew install")
...
for ((b=0; b<${#REGEX[@]}; b++)); do
	...
	[[ -z $SYSTEM ]] && red " 本脚本只支持 Debian、Ubuntu、CentOS 或 Alpine 系统,问题反馈:[https://github.com/fscarmen/warp_unlock/issues] " && exit 1
